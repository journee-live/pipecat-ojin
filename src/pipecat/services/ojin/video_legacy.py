"""Legacy OjinVideoService — kept as a fallback while consumers migrate to
the v2 module at ``pipecat.services.ojin.video``.

This module exposes the four-strategy ``InterruptStrategy`` enum
(``SMOOTH``, ``INSTANT_CUT``, ``SMOOTH_VIDEO_HARD_AUDIO``, ``FADE_OUT``) and
the offset-based single-buffer interruption model. The v2 module supports
fadeout-only with a per-TTS audio buffer queue; see
``demo-modal-agents/docs/ojin_video_service_redesign.md``.

New code should import from ``pipecat.services.ojin.video``.
"""
